...
class OtpVisitRepository {
  ...
  Future<void> startVisit(
      {required String bookingId, required String otp}) async {
    final response = await _client.functions
        .invoke('otp-start', body: {'booking_id': bookingId, 'otp': otp});
    if (response.status != 200) {
      throw StateError('Could not start visit: ${response.data}');
    }
  }
  ...
  Future<void> endVisit(
      {required String bookingId, required String otp}) async {
    final response = await _client.functions
        .invoke('otp-end', body: {'booking_id': bookingId, 'otp': otp});
    if (response.status != 200) {
      throw StateError('Could not end visit: ${response.data}');
    }
  }
  ...
  /// human review via the admin dashboard's AI review queue.
  Future<String?> submitVisitSummary(
      {required String bookingId, required String rawNotes}) async {
